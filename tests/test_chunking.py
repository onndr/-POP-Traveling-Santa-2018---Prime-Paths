...
def test_edges():
    ...
    assert len(chunks.edges[0, 0]) == 2
    assert len(chunks.edges[1, 0]) == 1
    assert len(chunks.edges[0, 1]) == 1

    assert chunks.connected[0, 0] is True
    assert chunks.connected[1, 0] is True
    assert chunks.connected[0, 1] is True

    chunks.clear_edge_info()

    assert chunks.edges[0, 0] is None
    assert chunks.edges[0, 1] is None
    assert chunks.edges[1, 0] is None

    assert chunks.connected[0, 0] is False
    assert chunks.connected[1, 0] is False
    assert chunks.connected[0, 1] is False
